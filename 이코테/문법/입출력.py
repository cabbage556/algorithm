...
#       데이터 입력 시 사용
#       한 줄의 문자열을 입력받음
#       입력받은 데이터를 정수형 데이터로 처리하기 위해 문자열을 정수로 바꾸는 int() 함수 사용
#       여러 개의 데이터를 입력 받는 경우, 데이터가 공백으로 구분되는 경우가 많음
#           입력받은 문자열을 띄어쓰기로 구분해 각각 정수 자료형 데이터로 저장하기
#               list(map(int, input().split()))

# 데이터 개수 입력
n = int(input())

# 각 데이터를 공백으로 구분하여 입력, 각 데이터를 정수형 데이터로 저장
...
#       문자열과 수를 함께 출력하는 경우
#       단순하게 더하기 연산자를 사용해 문자열과 수를 더하면 에러가 발생함
#       str() 함수를 사용해 문자열로 변경하거나, 각 자료형을 콤마로 구분하여 출력한다.
ans = 10
