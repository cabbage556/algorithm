...
# 코테에서 크기가 N이고, 모든 값이 0인 1차원 리스트 초기화
n = 10
a = [0] * n
print(a)
...
# 슬라이싱: 리스트에서 연속적인 위치를 갖는 원소를 가져오기 위해 사용함
#       대괄호 안에 콜론을 넣어 시작 인덱스와 (끝 인덱스 - 1)을 지정함
#       리스트를 슬라이싱하면 리스트가 반환됨
b = a[1:4]  # 두 번째 원소부터 네 번째 원소까지 슬라이싱
print(b)  # [2, 3, 4]
...
# 1부터 9까지 수의 제곱 값을 포함하는 리스트 초기화
a = [i * i for i in range(10)]
print(a)  # [0, 1, 4, 9, 16, 25, 36, 49, 64, 81]

# 코테에서 특정 크기의 2차원 리스트를 초기화할 때 리스트 컴프리헨션이 효과적임
...
#       3개의 리스트에서 인덱스 1에 해당하는 원소들이 5로 변경됨
...
# remove(특정 값)
#       리스트에서 특정 값을 갖는 원소를 제거함, 만약 여러 개라면 첫 번째만 제거함, O(N)
a.remove(3)  # 첫 번째, 3인 원소 제거
print(a)

# 코테에서 리스트 메서드 사용 시 주의할 점
#       insert() 메서드의 시간 복잡도는 O(N)임 -> 남발하면 시간 초과
#           인덱스에 원소 삽입 후 나머지 원소들의 위치를 조절해야 하므로 O(N)
#       remove() 메서드의 시간 복잡도는 O(N)임 -> 남발하면 시간 초과
#           원소 제거 후 나머지 원소들의 위치를 조절해야 하므로 O(N)

# 파이썬에서 특정 값을 갖는 모든 원소를 제거하는 방법
a = [1, 2, 3, 4, 5, 5, 5]
rmv_set = {3, 5}
